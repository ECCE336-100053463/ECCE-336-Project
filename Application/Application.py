...
def AccessRegistrationSystem():
    ...
    if Username != "":
        System[Username] = [Password, fullName.get(), number.get(),
                            email.get()]
...
def RecordedFeed():
    ...
    videoplayer.load(
        r"C:\Users\Khalid\ECCE-336-Project-1\Y2Mate.is - Twins climb out of cots-f_rQqVkEXOU-360p-1654154339715.mp4")
...
class PageOne(CTkFrame):
    def __init__(self, master):
        ...
        global username
        username = StringVar()
        CTkLabel(self, text="Username:", anchor="w").pack(
            side="top", fill="x", pady=10)
        entry_1 = CTkEntry(master=self, placeholder_text="",
                           textvariable=username)
        entry_1.pack()

        global password
        password = StringVar()
        CTkLabel(self, text="Password:", anchor="w").pack(
            side="top", fill="x", pady=10)
        entry_1 = CTkEntry(master=self, placeholder_text="",
...
class PageTwo(CTkFrame):
    def __init__(self, master):
        ...
        CTkLabel(self, text="Full Name:", anchor="w").pack(
            side="top", fill="x")
        entry_1 = CTkEntry(master=self, placeholder_text="",
                           textvariable=fullName).pack()

        CTkLabel(self, text="Phone Number:", anchor="w").pack(
            side="top", fill="x")
        entry_2 = CTkEntry(master=self, placeholder_text="",
                           textvariable=number).pack()

        CTkLabel(self, text="Email:", anchor="w").pack(side="top", fill="x")
        entry_3 = CTkEntry(master=self, placeholder_text="",
                           textvariable=email).pack()

        CTkLabel(self, text="Username:", anchor="w").pack(side="top", fill="x")
        entry_4 = CTkEntry(master=self, placeholder_text="",
                           textvariable=username).pack()

        CTkLabel(self, text="Password:", anchor="w").pack(side="top", fill="x")
        entry_5 = CTkEntry(master=self, placeholder_text="",
                           show="*", textvariable=password).pack()

        CTkLabel(self, text="Confirm Password:",
                 anchor="w").pack(side="top", fill="x")
        entry_6 = CTkEntry(master=self, placeholder_text="",
                           show="*", textvariable=password2).pack()
...
class PageFive(CTkFrame):
    def __init__(self, master):
        ...
        CTkFrame.__init__(self, master)
        CTkLabel(self, text="Notification Feed:").pack(
            side="top", fill="x", pady=10)

        text_1 = CTkTextbox(master=self, width=200, height=140)
        text_1.pack(pady=10, padx=10)
        text_1.insert("end", "Status: \n")

        if detectEmergency == True:
            text_1.insert("end", "Alert! Contacting parents...")
            text_1.insert("end", "Contacting authorities...")
        else:
            text_1.insert("end", "Child is safe...")
...
class PageSix(CTkFrame):
    def __init__(self, master):
        CTkFrame.__init__(self, master)
        CTkLabel(self, text="Recorded Camera Feed:").pack(
            side="top", fill="x", padx=10)
        CTkButton(self, text="Back to Camera Feed",
                  command=CameraFeed).pack(padx=10)
